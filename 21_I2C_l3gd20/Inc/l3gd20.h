...
/*****************************************************************
 ...
 PINS:
 	 * SCL	-> PB8 (external 4.7kohm pull up)
 	 * SDA 	-> PB9 (external 4.7kohm pull up)
 	 * SA0	->	+3v (SAD = 1)
 	 * CS	->  +3v
 	 * DRDY
 	 * INT1
  ...
  *-----------------------------------------------------------------*
  ...
  |---------|---------------|---------------|-------|---------------|
  | READ 	| 110101(0x6A)	| 		0		|	1	| 11010101(D5)	|
  |---------|---------------|---------------|-------|---------------|
  | WRITE	| 110101(0x6A)	|		0		|	0	| 11010100(D4)	|
  |---------|---------------|---------------|-------|---------------|
  | READ	| 110101(0x6B)	|		1		|	1	| 11010111(D7)	|
  |-------------------------|---------------|-------|---------------|
  | WRITE	| 110101(0x6B)	|		1		|	0	| 11010110(D6)	|
  *---------*---------------*---------------*-------*---------------*
...
#define SLAVE_ADDR 		(0x6B)	// pin SDO(SA0) is connected to Vdd externally
#define STANDBY			(0x08)	// used in CTRL_REG1 for power modes - standby = PD on while data from axes is disabled
#define	NORMAL			(0x0F)
#define WHO_AM_I_R		(0x0F)	// *WHO_AM_I = (0xD7) 0b1101 0111
